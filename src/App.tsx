  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/unauthorized-domain') {
        setAuthError("Este domínio não está autorizado no Firebase. Por favor, adicione-o nas configurações de Authentication.");
      } else {
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6">
        <div className="text-center space-y-8 max-w-sm w-full">
          <div className="w-20 h-20 bg-green-500 rounded-3xl flex items-center justify-center mx-auto shadow-2xl shadow-green-500/20">
            <Package className="w-10 h-10 text-black" />
          </div>
          <div className="space-y-2">
            <h1 className="text-4xl font-black tracking-tighter text-white uppercase">
              {isRegistering ? 'Criar Conta' : 'Controle de Fretes'}
            </h1>
            <p className="text-zinc-500 font-medium">
              {isRegistering ? 'Cadastre-se para gerenciar seus fretes' : 'Gestão de Carregamentos Agro'}
            </p>
          </div>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4 text-left">
            {isRegistering && (
              <div className="space-y-1">
                <label className="text-[10px] text-zinc-500 uppercase font-bold ml-2">Nome Completo</label>
                <input 
                  type="text" 
                  value={displayName}
                  ...
                  required
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-3 text-white focus:border-green-500 outline-none transition-all"
                />
              </div>
            )}
            <div className="space-y-1">
              <label className="text-[10px] text-zinc-500 uppercase font-bold ml-2">E-mail</label>
              <input 
                type="email" 
                value={email}
            ...
            <div className="space-y-1">
              <label className="text-[10px] text-zinc-500 uppercase font-bold ml-2">Senha</label>
              <input 
                type="password" 
                value={password}
            ...
            <button 
              type="submit"
              className="w-full bg-green-500 text-black font-bold py-4 rounded-2xl hover:bg-green-400 transition-all shadow-lg mt-2"
            >
          ...
          <div className="relative py-4">
            ...
            <div className="relative flex justify-center text-[10px] uppercase font-bold text-zinc-600 bg-black px-4">Ou continue com</div>
          </div>

          <button onClick={handleLogin} className="w-full bg-zinc-900 text-white font-bold py-4 rounded-2xl border border-zinc-800 hover:bg-zinc-800 transition-all flex items-center justify-center gap-3">
